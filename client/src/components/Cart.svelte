<script>
  ...
  async function buyStuff() {
    if ($tokens > $tokensRequired) {
      ...
      try {
        const { data } = await axios.post("/api/items/buyStuff", {
          cartItems: $cartItems,
        });
        $cartItems = [];
...
<div class="container">
    ...
    <table class="table container box">
      ...
      <tfoot>
        <tr>
          ...
          <th />
          <th
            ><button class="button is-primary is-light" on:click={buyStuff}
              >🛒 Buy</button
            >
          </th></tr
        >
        <tr>
          ...
          <th />
          <th
            ><a class="button is-primary is-light" href="#/generateTokens"
              >Update Tokens</a
            ></th
          >
        </tr>
  ...
  {:else}<hr />
    <a
      class="button is-primary columns is-mobile is-centered"
      href="#/dashboard"
